...
public class MusicList {
    ...
    private ArrayList<String> AlldataFiles;

    //mock up section--->>

    public void setLibrary(ArrayList<Song> library) {
    ...
    private ArrayList<ArrayList<String>> playlistCollection;
    ...
    public void buildPlaylistStructure(ComboBox comboBox) {
        ...
        for (String str : observableList) {
            if (!str.equals("Library") && !str.equals("New Playlist")) {
                String playlist = "./" + str + ".data";
            }
        }
        //fill in the .data files
    }

    private void buildArrayToAdd(String playlist) {
        Read read = new Read();
        read.setListOfPath(playlist);
        playlistCollection.add(read.getListOfPath());
    }
    ...
    public ArrayList<Song> getSubset() {
        return subset;
    }
    ...
    private ArrayList<String> newDataFile;
    private ArrayList<String> shuffleStates;
    private ComboBox selection;

    int counter;
    private String universal;
    public void subSet(ArrayList<String> subset, ComboBox selection, ArrayList<String> shuffle, ArrayList<ArrayList<String>> collection) {
        this.shuffleStates = shuffle;
        this.subset.clear();
        for (String path : subset) {
            if (path.contains(".data")) {

                Read read = new Read();
                read.setListOfPath(path);
                for(String str: read.getListOfPath()){
                    if (exist(str)) {
                        this.subset.add(getSongByLink(str));
                    }
                }
    ...
    public boolean dataMine(String playlist) {
        Read read = new Read();
        read.setListOfPath(playlist);

        for(String s : shuffleStates){
            String l = s.substring(s.indexOf("-")+1,s.length());
            String d = s.substring(s.indexOf(0) + 1, s.indexOf("-"));
            if(playlist.equals("./" + d + ".data") && l.equals("S")){
                //System.out.println(playlist + " " + d);
                return true;
            }
    ...
    public MusicList() {
        ...
        this.AlldataFiles = new ArrayList<>();
    }
    ...
    public void addSong(String name, String duration, String path) {
        ...
        //mockup
        this.library.add(new Song(name, duration, path));
    }
